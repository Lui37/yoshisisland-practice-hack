org interrupt_freespace ; everything in the interrupt region in bank 00 gets copied to WRAM 7E for execution, so adjust breakpoints accordingly

; "NMI" really refers to the IRQ at the bottom of the screen (irq_2) at the start of vblank
nmi:
    ; DP is $2100

	; backup the BG3 camera after the game code has run (e.g. for falling walls in 1-4 which modify $41)
	; also, modifying the BG3 camera can break OPT rendering, which this avoids
    LDA $0041 : STA !bg3_cam_x_backup
	LDA $0042 : STA !bg3_cam_x_backup+1
    LDA $0043 : STA !bg3_cam_y_backup
	LDA $0044 : STA !bg3_cam_y_backup+1

	LDA !hud_enabled
    AND !hud_displayed
	BNE .nmi_with_hud

    ; no HUD, so just set the registers that were skipped in the hijack and return
	REP #$20
	LDA !r_reg_tm_mirror : STA $2C
	LDA !r_reg_bg12nba_mirror : STA $0B

	SEP #$20
	LDA !r_reg_bg3sc_mirror : STA $09
	LDA !r_reg_bgmode_mirror : STA $05

	RTS

.nmi_with_hud
	; set 8x8 BG3 tile size, BG3 priority, mode 1
	LDA !r_reg_bgmode_mirror
	AND #%10110000 ; DCBA emmm, D/C/B/A = BG4/3/2/1 tile size (0=8x8; 1=16x16), e = mode 1 BG3 priority bit, mmm = BG mode
	ORA #%00001001
	STA $05
	
	; set BG3 tile character address to word $6000 ($C000)
	LDA !r_reg_bg34nba_mirror
	AND #%11110000 ; bbbb aaaa, bbbb = Base address for BG4, aaaa = Base address for BG3
	ORA #%00000110
	STA $0C

    ; set BG3 tilemap address to word $6400 ($C800)
	LDA #$64 ; !r_reg_bg3sc_mirror  aaaa aayx, a = map address>>10, x = horizontal flip, y = vertical flip
	STA $09

	REP #$20
	
	; set BG3 as a main screen
	LDA !r_reg_tm_mirror
	ORA #$0004 ; ---o 4321, 1/2/3/4/o = Enable BG1/2/3/4/OBJ for display on the main screen
	AND #~$0400 ; unset BG3 as a subscreen
	STA $2C

	; DMA tilemap to VRAM $6400 ($C800)
	LDX #$80 : STX $15 ; !reg_vmain - increment after writing to $2119
	LDX #$7E : STX !reg_a1b0 ; source bank
	LDA #$6400 : STA $16 ; !reg_vmadd - VRAM destination
	LDA #$1801 : STA !reg_dmap0 ; and !reg_bbad0 - set lower 8 bits of dest to $18 ($2118) and write 1 word increments
	LDA #!hud_buffer : STA !reg_a1t0l ; and !reg_a1t0h - hud buffer source address
	LDA #$00C0 : STA !reg_das0l ; and !reg_das0h - hud buffer size
	LDX #$01 : STX !reg_mdmaen ; enable DMA channel 0

	; set bg3 to (0,-9) TODO is this necessary with the hudma? assuming the hdma works properly, it shouldnt be
	SEP #$20
	; STZ $11 ; !reg_bg3hofs
	; STZ $11 ; write-twice
	; LDA !hud_vofs_lo : STA $12 ; !reg_bg3vofs - gets set to $FFF7, but 10bit so $03F7 (1015) out of $0400 (1024) i.e. -9
	; LDA !hud_vofs_hi : STA $12

	RTS

hud_hdma_table_h: ; put these tables here so they're available in work ram
	db $98 ; $80 (repeat bit) + 24 ($18) - num hud scanlines
	rep 24 : dw !hud_hofs ; 1 per line for $18 lines - kinda gross but necessary? TODO mb increase scanlines to prevent visual artifacts at bottom of hud
	db $00

hud_hdma_table_v:
	db $98
	rep 24 : dw !hud_vofs
	db $00

; apply the scroll values from the BG3 camera into the BG3 offset registers
restore_bg3_xy:
	LDA !bg3_cam_x_backup : STA !reg_bg3hofs
	LDA !bg3_cam_x_backup+1 : STA !reg_bg3hofs
	; LDA $43 : STA !reg_bg3vofs
	; LDA $44 : STA !reg_bg3vofs
	LDA !bg3_cam_y_backup : STA !reg_bg3vofs
	LDA !bg3_cam_y_backup+1 : STA !reg_bg3vofs
	RTS

check_lag:
	LDA $011B ; Full Game Mode completion flag - $00: Game Mode still running (set by NMI/IRQ), $FF: Game Mode complete (set by end of game loop)
	BNE .no_lag
	REP #$20
	INC !lag_counter
	SEP #$20
.no_lag
	INC !frames_passed
	LDA $011B
	RTS

load_irq2_vcount:
	STA !reg_htimel
	LDA !hud_enabled
    AND !hud_displayed
	BEQ .skip_hud_irq2
	LDA #!irq_v
	JMP $C457
.skip_hud_irq2
	LDA #$03
	STA !r_irq_count ; just increases the counter to skip over our custom IRQs - 03 points to irq_2b, which gets INCed again after the JMP
	LDA #!nmi_v
	JMP $C457

irq:
	LDA !r_irq_count
	ASL
	TAX
	JMP (.pointers,x)
.pointers
	dw $C417 ; irq_0
	dw $C440 ; irq_1
	dw irq_2a ; "restore" original graphics settings after HUD during H-blank
	dw irq_2b ; "restore" more things 2 lines later (restores are separate because there's a limit to amt of instructions per hblank?)
	dw $C465 ; irq_2 (NMI)

macro next_hblank()
-	BIT !reg_hvbjoy
	BVS - ; wait for hblank to end
-	BIT !reg_hvbjoy
	BVC - ; wait for hblank to start
endmacro

irq_2a:
	%next_hblank()
	
	; restore stuff (~40 cycles?)
	LDA !r_reg_bgmode_mirror : STA !reg_bgmode
	LDA !r_reg_bg3sc_mirror : STA !reg_bg3sc
	LDA !r_reg_bg34nba_mirror : STA !reg_bg34nba
	LDA !r_reg_tm_mirror : STA !reg_tm
	LDA !r_reg_ts_mirror : STA !reg_ts
	
	; next IRQ
	LDA #!irq_v+2 ; so irq_2a runs at the bottom of the hud, and irq_2b runs 2 scanlines later, and prob causes some graphical glitches in those 2 lines
	JMP $C431

irq_2b:
	%next_hblank()

	; TODO should we disable the hud hdma channels here and re-enable in nmi?
	
	; restore x/y scrolls (~32 cycles?)
	JSR restore_bg3_xy
	
	; next IRQ
	LDA #!nmi_v
	JMP $C431


warnpc interrupt_freespace+$7F0
